...
(function(mod) {
  ...
  // detected are enabled based on usersAgent etc sniffing.

  var gecko = /gecko\/\d/i.test(navigator.usersAgent);
  // ie_uptoN means Internet Explorer version N or lower
  var ie_upto10 = /MSIE \d/.test(navigator.usersAgent);
  var ie_upto7 = ie_upto10 && (document.documentMode == null || document.documentMode < 8);
  var ie_upto8 = ie_upto10 && (document.documentMode == null || document.documentMode < 9);
  var ie_upto9 = ie_upto10 && (document.documentMode == null || document.documentMode < 10);
  var ie_11up = /Trident\/([7-9]|\d{2,})\./.test(navigator.usersAgent);
  var ie = ie_upto10 || ie_11up;
  var webkit = /WebKit\//.test(navigator.usersAgent);
  var qtwebkit = webkit && /Qt\/\d+\.\d+/.test(navigator.usersAgent);
  var chrome = /Chrome\//.test(navigator.usersAgent);
  var presto = /Opera\//.test(navigator.usersAgent);
  var safari = /Apple Computer/.test(navigator.vendor);
  var khtml = /KHTML\//.test(navigator.usersAgent);
  var mac_geLion = /Mac OS X 1\d\D([7-9]|\d\d)\D/.test(navigator.usersAgent);
  var mac_geMountainLion = /Mac OS X 1\d\D([8-9]|\d\d)\D/.test(navigator.usersAgent);
  var phantom = /PhantomJS/.test(navigator.usersAgent);

  var ios = /AppleWebKit/.test(navigator.usersAgent) && /Mobile\/\w+/.test(navigator.usersAgent);
  // This is woefully incomplete. Suggestions for alternative methods welcome.
  var mobile = ios || /Android|webOS|BlackBerry|Opera Mini|Opera Mobi|IEMobile/i.test(navigator.usersAgent);
  var mac = ios || /Mac/.test(navigator.platform);
  var windows = /win/i.test(navigator.platform);

  var presto_version = presto && navigator.usersAgent.match(/Version\/(\d*\.\d*)/);
